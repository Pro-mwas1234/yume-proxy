/**
 * YumeZone — HLS Proxy Worker (Base64 edition)
 *
 * Routes:
 *   GET /p/<base64url>   — primary (url + optional referer, base64url encoded)
 *   GET /proxy?url=...   — legacy plain-text (backward compat)
 *   GET /health
 */
  ...
  return lines.map(raw => {
    ...
    if (line.startsWith('#') && line.includes('URI="')) {
      return line.replace(/URI="([^"]+)"/g, (_, uri) => {
        ...
        return `URI="${WORKER_BASE}/p/${encodePayload(abs, referer)}"`;
      });
    }
    ...
    if (line && !line.startsWith('#')) {
      ...
      return `${WORKER_BASE}/p/${encodePayload(abs, referer)}`;
    }
